...
@php
    ...
    $tabs = [
        ...
        'payments' => ['Pembayaran PayKita', 'topup', 'Satu-satunya gateway eksternal untuk checkout produk dan isi saldo.'],
    ...
    $pk = fn($key,$default='') => old($key, $values['paykita.'.$key] ?? $default);
...
<div class="mt-6 grid gap-6 xl:grid-cols-[1fr_340px]">
    @if($tab === 'payments')
        <form class="card space-y-6 p-6" method="post" action="{{ route('admin.settings.update') }}">
            @csrf @method('put')
            <input type="hidden" name="group" value="paykita">
            <div>
                <div class="flex items-center gap-3"><span class="grid size-11 place-items-center rounded-2xl bg-violet-500/10 text-violet-600 dark:text-violet-300"><x-icon name="topup" /></span><div><h2 class="text-lg font-black">PayKita</h2><p class="text-sm text-slate-500">Gateway eksternal tunggal untuk seluruh transaksi baru.</p></div></div>
            </div>
            <div><label class="label">URL dasar</label><input class="input" type="url" name="base_url" value="{{ $pk('base_url','https://paykita.biz.id') }}" required></div>
            <div><label class="label">API key project</label><x-password-input name="api_key" value="" placeholder="Kosongkan untuk mempertahankan API key saat ini" /><p class="mt-1 text-xs text-slate-500">Gunakan pk_live_... milik project Dapet OTP. Key disimpan terenkripsi dan tidak dikirim ke browser.</p></div>
            <div><label class="label">Masa aktif pembayaran (detik)</label><input class="input" type="number" min="60" max="86400" name="ttl_seconds" value="{{ $pk('ttl_seconds',600) }}" required></div>
            <div class="card-soft p-4 text-sm leading-6 text-slate-500">Checkout produk dapat dibayar langsung melalui PayKita atau menggunakan saldo akun. Isi saldo tetap tersedia sebagai opsi, tetapi tidak lagi wajib sebelum membeli produk.</div>
            <button class="btn-primary">Simpan PayKita</button>
        </form>
    @else
        <form class="card space-y-5 p-6" method="post" action="{{ route('admin.settings.update') }}" @if($tab==='site') enctype="multipart/form-data" @endif>
                ...
                <div class="rounded-3xl border border-slate-200 p-5 dark:border-white/10">
                    ...
                    <div class="mt-4 grid max-w-2xl gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input class="input" type="number" name="logo_mobile_shift" min="-5" max="30" step="1" value="{{ max(-5, min(30, (int) $v('logo_mobile_shift', 0))) }}">
                            <p class="mt-1 text-xs text-slate-500">Nilai negatif menggeser sedikit ke kiri, nilai positif ke kanan. Default 0%. Rentang dibatasi agar logo tidak terpotong di HP.</p>
                        </div>
                    </div>
            ...
            @elseif($tab==='topup')
                ...
                <div class="card-soft p-4 text-sm text-slate-500">Isi saldo memakai PayKita QRIS. Pembelian produk tetap dapat dibayar langsung tanpa isi saldo.</div>
    ...
    <aside class="space-y-4">
        <section class="card p-5">
            ...
            <div class="mt-4 grid gap-2">
                ...
                @elseif($tab==='payments')
                    <form method="post" action="{{ route('admin.settings.test-paykita') }}">@csrf<button class="btn-secondary w-full justify-between">Tes PayKita <x-icon name="arrow-right" size="size-4" /></button></form>
                @elseif($tab==='mail')

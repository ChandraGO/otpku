...
<section class="page-grid mx-auto flex min-h-[calc(100vh-8rem)] items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
        <div class="card w-full p-6 sm:p-8">
            <div class="flex items-start gap-4"><span class="grid size-12 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-violet-600 to-cyan-500 text-white shadow-lg"><x-icon name="user" size="size-6" /></span><div><h1 class="text-2xl font-black">Masuk ke akun</h1></div></div>
            ...
            <form class="space-y-4" method="post" action="{{ route('login') }}">
                @csrf
                <div><label class="label" for="login">Email / nama pengguna</label><input id="login" class="input @error('login') !border-rose-400 !ring-4 !ring-rose-500/10 @enderror" name="login" value="{{ old('login') }}" autocomplete="username" placeholder="nama@email.com atau nama pengguna" required autofocus>@error('login')<p class="mt-2 text-xs font-semibold text-rose-600 dark:text-rose-300">Periksa kembali email/nama pengguna dan kata sandi Anda.</p>@enderror</div>
                <div><div class="flex justify-between gap-3"><label class="label" for="password">Kata sandi</label><a class="text-xs font-semibold text-brand-600 dark:text-brand-300" href="{{ route('password.request') }}">Lupa kata sandi?</a></div><x-password-input id="password" name="password" autocomplete="current-password" placeholder="Masukkan kata sandi" required /></div>
                ...
                <button class="btn-primary w-full py-3">Masuk <x-icon name="arrow-right" size="size-4" /></button>
            </form>

...
<section class="mx-auto max-w-xl px-4 py-12">
    <div class="card p-6 sm:p-8">
        ...
        <p class="mt-2 text-sm text-slate-500">OTP verifikasi akan dikirim langsung ke email setelah pendaftaran.</p>
        <x-flash />
        <form class="mt-7 grid gap-4 sm:grid-cols-2" method="post" action="{{ route('register') }}">
            @csrf
            <div><label class="label">Username</label><input class="input" name="username" value="{{ old('username') }}" autocomplete="username" required></div>
            ...
            <div><label class="label">Password</label><x-password-input name="password" autocomplete="new-password" required /></div>
            <div><label class="label">Ulangi password</label><x-password-input name="password_confirmation" autocomplete="new-password" required /></div>

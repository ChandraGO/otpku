...
<section class="lg:hidden">
    <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#1e293b] via-[#263859] to-[#111827] p-5 text-white shadow-xl">
        ...
        <div class="relative z-10 flex items-start justify-between gap-4">
            <div>
                <span class="badge bg-white/10 text-cyan-200">Fast & Secure OTP Codes</span>
        ...
        <div class="relative z-10 mt-5 flex gap-3"><a href="{{ route('services.index') }}" class="btn-primary flex-1">Pilih layanan</a><a href="{{ route('topups.index') }}" class="btn-secondary !border-white/20 !bg-white/10 !text-white">Top Up</a></div>
    </div>
    ...
    <a href="{{ route('services.index') }}" class="mt-5 flex items-center gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#0f1729]">
        ...
        <span class="flex-1 text-sm font-semibold text-slate-400">Search services</span>
...
<section class="mt-8">
    <div class="flex items-center justify-between gap-4"><div><h2 class="text-2xl font-black">Recommendation</h2><p class="mt-1 text-sm text-slate-500">Layanan dengan stok terbesar saat ini.</p></div><a href="{{ route('services.index') }}" class="text-sm font-bold text-violet-600 dark:text-violet-300">Semua</a></div>
    <div class="scrollbar-thin mt-5 flex gap-2 overflow-x-auto pb-2 lg:hidden">
        <a href="{{ route('services.index', ['sort' => 'popular']) }}" class="filter-chip filter-chip-active">Popular</a><a href="{{ route('services.index', ['sort' => 'price_asc']) }}" class="filter-chip">Termurah</a><a href="{{ route('services.index', ['sort' => 'stock']) }}" class="filter-chip">Stok besar</a><a href="{{ route('services.index', ['sort' => 'name']) }}" class="filter-chip">A–Z</a>
    </div>
    <div class="mt-5 grid gap-3 lg:grid-cols-2 2xl:grid-cols-4">
        @forelse($featuredServices as $service)
            <a href="{{ route('services.show', $service) }}" class="service-row">
                ...
                <div class="min-w-0 flex-1"><div class="truncate font-black">{{ trim($service->name) }}</div><div class="mt-1 text-xs text-slate-500">Lowest: <span class="font-bold text-cyan-600 dark:text-cyan-300">Rp {{ number_format((float) $service->lowest_price, 0, ',', '.') }}</span></div><div class="mt-1 text-xs text-slate-400">Highest: Rp {{ number_format((float) $service->highest_price, 0, ',', '.') }} · Stock {{ number_format((int) $service->total_stock) }}</div></div><x-icon name="chevron-right" class="text-slate-400" />
            </a>

...
<div x-data="{ filters: {{ request()->hasAny(['country','sort','stock']) ? 'true' : 'false' }} }">
    ...
    <form method="get" class="mt-7" data-auto-filter>
        <div class="flex gap-3">
            <label class="relative block flex-1">
                <x-icon name="search" class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-cyan-500" />
                <input class="input !rounded-3xl !py-4 !pl-12" name="q" value="{{ request('q') }}" placeholder="Cari layanan..." autocomplete="off">
            </label>
        ...
        <div x-show="filters" class="mt-4 sm:block">
            <div class="card grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div class="flex items-end">
                    <a href="{{ route('services.index') }}" class="btn-secondary w-full">Atur ulang</a>
                </div>
            </div>
        </div>
    </form>
            ...
            <a href="{{ route('services.show', $service) }}" class="service-row">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500">
                        <span>Termurah: <strong class="text-cyan-600 dark:text-cyan-300">Rp {{ number_format((float) $service->lowest_price, 0, ',', '.') }}</strong></span>
                        <span>·</span>
                        <span>Tertinggi: Rp {{ number_format((float) $service->highest_price, 0, ',', '.') }}</span>
                    </div>
    ...
    <div class="table-wrap mt-6 hidden lg:block">
        <table class="table">
            <thead><tr><th>Layanan</th><th>Harga termurah</th><th>Harga tertinggi</th><th>Stok</th><th>Pilihan</th><th></th></tr></thead>
            <tbody>
                @forelse($services as $service)

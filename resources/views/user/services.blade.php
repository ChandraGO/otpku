...
<div x-data="{ filters: {{ request()->hasAny(['country','sort','stock']) ? 'true' : 'false' }} }">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
            ...
            <h1 class="section-title mt-4">Daftar Layanan</h1>
            <p class="section-copy">Cari layanan, bandingkan harga, lalu pilih negara dan nomor yang tersedia.</p>
        </div>
        <div class="hidden rounded-3xl border border-violet-200 bg-white px-5 py-3 text-right shadow-sm dark:border-violet-400/20 dark:bg-white/5 sm:block">
            <div class="text-[10px] font-black uppercase tracking-wider text-slate-400">Saldo aktif</div>
            <div class="mt-1 text-lg font-black text-violet-700 dark:text-violet-300">Rp {{ number_format((float) auth()->user()->balance, 0, ',', '.') }}</div>
        </div>
    </div>

    <form action="{{ route('services.index') }}" method="get" data-live-catalog-form class="mt-7">
        <div class="flex gap-3">
            <label class="relative block flex-1">
                <x-icon name="search" class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-cyan-500" />
                <input class="input !rounded-3xl !py-4 !pl-12" type="search" name="q" value="{{ request('q') }}" placeholder="Cari layanan..." autocomplete="off">
            </label>
            <button type="button" @click="filters = !filters" class="btn-secondary !rounded-3xl !px-4 sm:hidden" aria-label="Buka filter"><x-icon name="filter" /></button>
        </div>
        ...
        <div x-show="filters" class="mt-4 sm:block">
            <div class="card grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div class="flex items-end"><a href="{{ route('services.index') }}" class="btn-secondary w-full">Atur ulang</a></div>
            </div>
        </div>
    </form>
    <p class="mt-2 text-xs text-slate-500" data-live-catalog-status aria-live="polite">Hasil akan diperbarui otomatis.</p>
    ...
    <div data-live-catalog-results>
            ...
            @forelse($services as $service)
                <a href="{{ route('services.show', $service) }}" class="service-row">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="mt-1 text-xs text-slate-500">Harga Rp {{ number_format((float) $service->lowest_price, 0, ',', '.') }} – Rp {{ number_format((float) $service->highest_price, 0, ',', '.') }}</div>
                        <div class="mt-1 text-xs font-semibold text-slate-400">Stok {{ number_format((int) $service->total_stock) }} · {{ number_format((int) $service->available_variants) }} pilihan</div>
                    </div>
        ...
        <div class="table-wrap mt-6 hidden lg:block">
            <table class="table">
                <thead><tr><th>Layanan</th><th>Harga terendah</th><th>Harga tertinggi</th><th>Stok</th><th>Pilihan</th><th></th></tr></thead>
                <tbody>
                    @forelse($services as $service)
                        <tr class="transition hover:bg-violet-50/60 dark:hover:bg-white/[.025]">
                            ...
                            <td><span class="badge bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300">{{ number_format((int) $service->total_stock) }} unit</span></td>

...
<section class="card p-5"><h2 class="text-lg font-black">Riwayat isi saldo</h2><div class="mt-4 table-wrap"><table class="table"><thead><tr><th>ID Pesanan</th><th>Nominal</th><th>Penyedia</th><th>Metode</th><th>Status</th><th></th></tr></thead><tbody>@forelse($topups as $topup)<tr><td class="font-mono text-xs">{{ $topup->order_id }}</td><td class="font-bold">Rp {{ number_format((float)$topup->amount,0,',','.') }}</td><td>{{ $topup->gateway === 'paykita' ? 'QRIS' : ucfirst($topup->gateway ?: 'QRIS') }}</td><td>{{ strtoupper(str_replace('_',' ',$topup->payment_method)) }}</td><td><x-status :value="$topup->status" /></td><td><a class="btn-secondary !px-3 !py-2" href="{{ route('topups.show',$topup) }}">Detail</a></td></tr>@empty<tr><td colspan="6" class="py-10 text-center text-slate-500">Belum ada isi saldo.</td></tr>@endforelse</tbody></table></div><div class="mt-5">{{ $topups->links() }}</div></section>
</div>
@endsection

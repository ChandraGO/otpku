    ...
    <div class="mt-6 space-y-3 lg:hidden">
        @forelse($prices as $price)
            <article class="card p-5">
                ...
                <div class="mt-5 flex items-end justify-between gap-4"><div><div class="text-xs font-bold uppercase tracking-wider text-slate-400">Harga</div><div class="mt-1 text-2xl font-black text-violet-700 dark:text-violet-300">Rp {{ number_format((float) $price->sell_price, 0, ',', '.') }}</div></div><form method="post" action="{{ route('orders.store') }}">@csrf<input type="hidden" name="price_id" value="{{ $price->id }}"><input type="hidden" name="idempotency_key" value="{{ (string) Str::uuid() }}"><button class="btn-primary" @disabled($price->stock < 1)>Pesan</button></form></div>
            </article>

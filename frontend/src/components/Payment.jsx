...
export const PaymentPanel = ({ topup, onClose, onChange, note, mode = "topup" }) => {
  const [tick, setTick] = useState(Date.now());
  const [busy, setBusy] = useState(false);
  const direct = mode === "direct";
  const endpoint = direct ? "/order-payments" : "/topups";
  ...
  const shouldPoll = topup.status === "pending" || (direct && topup.status === "paid" && !topup.order_id);

  useEffect(() => {
    if (!shouldPoll) return undefined;
    const check = () => {
      http.get(`${endpoint}/${topup.id}`).then(({ data }) => onChange?.(data)).catch(() => {});
    };
    const iv = setInterval(check, direct && topup.status === "paid" ? 2500 : 4000);
    return () => clearInterval(iv);
  }, [endpoint, topup.id, topup.status, topup.order_id, direct, shouldPoll]); // eslint-disable-line
  ...
  const statusText = useMemo(() => {
    if (!direct) return st.text;
    if (topup.status === "paid" && topup.order_id) return "Pesanan berhasil dibuat";
    if (topup.status === "paid") return "Pembayaran diterima";
    return st.text;
  }, [direct, st.text, topup.status, topup.order_id]);

  const cancel = async () => {
    setBusy(true);
    try {
      await http.post(`${endpoint}/${topup.id}/cancel`);
  ...
  return (
    <div data-testid={direct ? "direct-order-payment" : "topup-payment"} className="rise rounded-3xl border border-primary/40 bg-card p-5">
      <div className="flex flex-wrap items-center gap-3">
        <span className={`flex items-center gap-2 rounded-xl border px-3 py-1.5 text-[11px] font-bold uppercase ${st.tone}`} data-testid="payment-status">
          <st.icon className="h-3.5 w-3.5" /> {statusText}
        </span>
      ...
      {topup.status === "pending" && topup.payment_code ? (
        <div className="mt-4 text-center">
          {direct && <p className="mb-1 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">Bayar layanan langsung</p>}
      ...
      ) : direct && topup.status === "paid" ? (
        <div className="mt-5 rounded-2xl border border-border bg-muted/30 p-4 text-center">
          {topup.order_id ? (
            <>
              <CheckCircle2 className="mx-auto h-8 w-8 text-emerald-500" />
              <p className="mt-2 font-extrabold">Pesanan berhasil dibuat otomatis</p>
              <p className="mt-1 text-xs text-muted-foreground">Membuka halaman Pesanan…</p>
            </>
          ) : (
            <>
              <Loader2 className="mx-auto h-7 w-7 animate-spin text-primary" />
              <p className="mt-2 font-extrabold">Pembayaran diterima, sedang membuat pesanan</p>
              <p className="mt-1 text-xs text-muted-foreground">Saldo akun tidak ditambah. Sistem langsung memproses layanan yang dipilih.</p>
              {topup.fulfillment_error && (
                <p className="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-500">
                  {topup.fulfillment_error} Sistem akan mencoba lagi otomatis.
                </p>
              )}
            </>
          )}
